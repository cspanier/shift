...
BOOST_AUTO_TEST_CASE(rc_cache)
{
  ...
  write_tiff_image(settings.input_path / "test_image1.tif", 32, 32, 0xFF0077FF);
  write_tiff_image(settings.input_path / "test_image2.tif", 32, 32, 0xFF77FF00);
  write_tiff_image(settings.input_path / "test_image3.tif", 32, 32, 0xFFFF0077);
  write_text_file(settings.input_path / ".rc-rules.json",
                  R"({ "import": {
    "pass": 1,
    "action": "image-import-tiff",
    "input": {
      "image": "<input-path/><rule-path/>(test_image.*)\\.tif$"
    },
    "output": {
  ...
  auto check_files = [&]() {
    ...
    BOOST_CHECK(!fs::exists(settings.input_path / "test_image1.tif") ^
                fs::exists(settings.build_path / "test_image1.image_header"));
    BOOST_CHECK(!fs::exists(settings.input_path / "test_image2.tif") ^
                fs::exists(settings.build_path / "test_image2.image_header"));
    BOOST_CHECK(!fs::exists(settings.input_path / "test_image3.tif") ^
                fs::exists(settings.build_path / "test_image3.image_header"));

    BOOST_CHECK(
      fs::file_size(settings.output_path / ".index.json", error_code) == 294);
    BOOST_CHECK(
      fs::file_size(settings.output_path / "global.cache", error_code) == 134);
    BOOST_CHECK(
      !fs::exists(settings.input_path / "test_image1.tif") ^
      fs::exists(settings.output_path / "test_image1.lod_0.image_buffer"));
    BOOST_CHECK(
      !fs::exists(settings.input_path / "test_image2.tif") ^
      fs::exists(settings.output_path / "test_image2.lod_0.image_buffer"));
    BOOST_CHECK(
      !fs::exists(settings.input_path / "test_image3.tif") ^
  ...
  {
    // We need to sleep to make sure that the file that is rewritten below gets
    // a different timestamp. Otherwise the change might not be detected.
    std::this_thread::sleep_for(std::chrono::seconds(2));

    // Change content of a single image file.
    write_tiff_image(settings.input_path / "test_image2.tif", 32, 32,
                     0xFF123456);
    run_rc(settings, 2, 0);
    check_files();
