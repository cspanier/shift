/************************************************************************************/ /**
 ...
 /// \file
 ****************************************************************************************/
// This is the implementation of address-aligned malloc and free using a linked
// list (inserting at front). These functions are not thread-safe because there
// is an access to the addressList global variable. Use a mutex when you access
// the global variable to make it thread-safe.
#include <cstdlib>
#include <cstdio>

// a linked list node to store a coupled memory address of the original and
// aligned address.
...
//=================================================================================================================================
/// Takes the size of the buffer you would like  to allocate with the return
/// buffer starts on an alignment boundary (multiple of
///  alignment input specified).  The input alignment should be a power of 2.
///  The memory allocated through aligned_malloc has to be deallocated by
///  aligned_free.  Otherwise the result will be unspecified.
///
/// \param bytes     The number of bytes requested.
/// \param alignment The alignment required for the output address of the
/// memory.  This should be of a power of 2
///                  (i.e. 1, 2, 4, 8, 16, ...).
///
/// \return The starting address of the allocated memory.  The address will be
/// aligned to the specified request.
//=================================================================================================================================
void* aligned_malloc(size_t bytes, size_t alignment)
{
  ...
  if ((memory = malloc(totalSize)) == nullptr)
  {
    return nullptr;
  }

  // Compute the offset alignment to add to the address. Because alignment is
  // guaranteed to be power of 2,
  //  we do not need to use modulus operator.  The second line ensures that if
  //  the original address already address aligned, the offset should be 0.
  size_t offset;
  ...
  // Store a coupled entry of address and aligned address into a linked list so
  // we can free the memory with the correct
  //  address in aligned_free() function.  The new coupled entry is prepended
  //  into the linked list.  The next code makes sure that the linked list does
  //  not store multiple entry of the same memory address.
  node* addressEntry;
...
//=================================================================================================================================
// Round a number up to the next power of two number.  If the number is already
// a power of 2, it returns the same number. This function only works for 32
// bits number.
//
