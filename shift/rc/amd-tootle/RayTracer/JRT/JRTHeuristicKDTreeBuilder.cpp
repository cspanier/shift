...
using Split = JRTHeuristicKDTreeBuilder::Split;
using TriangleBB = JRTHeuristicKDTreeBuilder::TriangleBB;
using SplitVec = JRTHeuristicKDTreeBuilder::SplitVec;

/// This function is simply a predicate used with std::sort
bool SplitLess(const Split& s1, const Split& s2)
{
  // if we have a degenerate bounding box, force splits to be ordered min, then
  // max
  if (s1.value == s2.value && s1.nTriBB == s2.nTriBB)
  {
    return ((s1.bMaxSplit == 0u) && (s2.bMaxSplit != 0u));
  }
  else
  {
...
void SortBBs(std::uint32_t eAxis, std::vector<TriangleBB*>& rBBs)
{
...
void JRTHeuristicKDTreeBuilder::ExtractSplits(
  std::uint32_t eAxis, const std::vector<TriangleBB>& rBBs,
  std::vector<Split>& rSplits, const JRTBoundingBox& /*rSceneBounds*/)
{
  ...
  for (std::uint32_t i = 0; i < rBBs.size(); i++)
  {
...
void BuildBBs(const std::vector<const JRTTriangle*>& rTris,
              std::vector<TriangleBB>& rBBs)
{
...
void PartitionBBs(std::uint32_t eAxis, float fPosition,
...
void PartitionSplits(float /*fValue*/, std::uint32_t /*eAxis*/,
                     const SplitVec& rSplits,
                     const std::vector<TriangleBB>& rBBs, SplitVec& rBack,
                     SplitVec& rFront)
{
  rFront.reserve(rSplits.size());
  rBack.reserve(rSplits.size());

  for (std::uint32_t i = 0; i < rSplits.size(); i++)
  {
...
void JRTHeuristicKDTreeBuilder::LocateBestSplit(
  float /*fNodeBBInvArea*/, const JRTBoundingBox& rNodeBounds,
  const SplitVec splits[3], std::uint32_t axis, std::uint32_t nTriCount,
  float& fBestCost, bool& bSplit, std::uint32_t& eSplitAxis, float& fSplitValue)
{
  ...
  auto nSplits = (std::uint32_t)rSplitVec.size();
  ...
  // - for each 'opening' plane, we compute the cost, and then increment the #
  // of triangles
  //     on the 'behind' side of the plane (since the triangle will be behind
  //     all subsequent planes)
  //  For each 'closing' plane, we decrement the number of tris in front before
  //  computing the cost
  std::uint32_t nTrisBehind = 0;
  std::uint32_t nTrisInFront = nTriCount;
  std::uint32_t i = 0;
  ...
  // surface area of root bounding box.  we omit the multiplies by two since
  // they cancel out
  float farea =
    bbSizeU * bbSizeV + bbSize[axis] * bbSizeU + bbSize[axis] * bbSizeV;
  ...
  for (; i < nSplits; i++)
  {
    ...
    nTrisInFront -= (localSplit->bMaxSplit) != 0u ? 1 : 0;
    nTrisBehind += (localSplit->bMaxSplit) != 0u ? 0 : 1;

    // evaluate the cost of this split using the surface area heuristic

    // compute surface area for each side of the box.  Note that we deliberately
    // leave off the multiply by two since it cancels out
    float back_area =
      sa_const + (bbSizeU + bbSizeV) * (localSplit->value - fNodeMin);
    float front_area =
      sa_const + (bbSizeU + bbSizeV) * (fNodeMax - localSplit->value);
    float cost =
      1.0f + INTERSECT_COST * farea *
               ((back_area * nTrisBehind) + (front_area * nTrisInFront));
...
void JRTHeuristicKDTreeBuilder::ClassifyBBs(
  JRTHeuristicKDTreeBuilder::SplitVec& rSplits, std::vector<TriangleBB>& rBBs,
  std::uint32_t eAxis, float fValue)
{
  for (std::uint32_t i = 0; i < rSplits.size(); i++)
  {
    // classify only on max-splits for efficiency
    if (rSplits[i]->bMaxSplit != 0u)
    {
...
void JRTHeuristicKDTreeBuilder::BuildTreeRecursive(
  std::uint32_t nDepthLimit, const JRTBoundingBox& rNodeBounds,
  SplitVec splits[3], std::vector<TriangleBB*>& rBBsThisNode, JRTKDNode* pNode,
  std::vector<JRTKDNode>& rNodesOut, std::vector<std::uint32_t>& rTrisOut)
{
  auto nTriCount = (std::uint32_t)rBBsThisNode.size();

  JRT_ASSERT(splits[0].size() == splits[1].size() &&
             splits[1].size() == splits[2].size());
  ...
  bool bSplit = false;
  float fSplitValue = 0.0f;
  std::uint32_t eSplitAxis = X_AXIS;

  float fNodeBBInvArea = 1.0f / Max(0.0000001f, rNodeBounds.GetSurfaceArea());

  for (std::uint32_t axis = X_AXIS; axis <= Z_AXIS; axis++)
  {
    LocateBestSplit(fNodeBBInvArea, rNodeBounds, splits, axis, nTriCount,
                    fBestCost, bSplit, eSplitAxis, fSplitValue);
  }
  ...
  if (!bSplit || nDepthLimit == 0)
  {
    ...
    pNode->leaf.triangle_start = (std::uint32_t)rTrisOut.size();

    for (std::uint32_t i = 0; i < rBBsThisNode.size(); i++)
    {
  ...
  else
  {
    ...
    {
      PartitionSplits(fSplitValue, eSplitAxis, splits[j], m_bbs, backSplits[j],
                      frontSplits[j]);
    ...
    BuildTreeRecursive(nDepthLimit - 1, front_bounds, frontSplits, frontBBs,
                       &rNodesOut[nFront], rNodesOut, rTrisOut);
    BuildTreeRecursive(nDepthLimit - 1, back_bounds, backSplits, backBBs,
                       &rNodesOut[nBack], rNodesOut, rTrisOut);
  }
}

void JRTHeuristicKDTreeBuilder::BuildTreeImpl(
  ...
  std::vector<JRTKDNode>& rNodesOut, std::vector<std::uint32_t>& rTrisOut)
{
  ...
  BuildTreeRecursive(JRTKDTree::MAX_TREE_DEPTH, rBounds, splitPtrs, bbPtrs,
                     &rNodesOut[0], rNodesOut, rTrisOut);
}
