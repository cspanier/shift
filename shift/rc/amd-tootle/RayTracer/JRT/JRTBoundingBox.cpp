...
bool JRTBoundingBox::RayHit(const Vec3f& rOrigin, const Vec3f& rDirection,
                            float* tmin, float* tmax) const
{
  ...
  //  (Value - Origin) / Dc  where c is the axis and value is the plane's
  //  position on that axis Note that value is the negative of the D term of the
  //  plane eqn.
  ...
  *tmax = FLT_MAX;
  ...
  for (int i = 0; i < 3; i++)
  {
    // TODO: this will be slow due to compiler dumping the xmm reg to memory.
    // FIX it
...
void JRTBoundingBox::Split(unsigned int axis, float value,
                           JRTBoundingBox& front, JRTBoundingBox& back) const
{
  front = *this;
  back = *this;
...
{
  Vec3f diff = m_max - m_min;
  float area = (2.0f * diff.x * diff.y) + (2.0f * diff.x * diff.z) +
               2.0f * (diff.y * diff.z);
...
//=================================================================================================================================
/// Given a point and a plane equation, compute the side of where the point lies
/// (1 if it is the same side as the normal,
///  -1 if it is not the same side, and 0 if the point lies on the plane).
///
/// \param plane      The plane equation.
/// \param rPoint     The point.
///
/// \return 1 if the point lies on the same side of the normal, -1 if it is not
/// the same side, 0 if the point lies on the plane
//=================================================================================================================================
...
//=================================================================================================================================
static void ConstructPlane(const Vec3f& rPoint0, const Vec3f& rPoint1,
                           const Vec3f& rPoint2, float fPlane[4])
{
...
//=================================================================================================================================
/// Intersection test between a triangle and an axis-aligned box.  It was based
/// on a paper
///  from
...
//=================================================================================================================================
bool JRTBoundingBox::TriangleIntersect(const Vec3f* pV1, const Vec3f* pV2,
                                       const Vec3f* pV3) const
{
  ...
  // see:
  // http://fileadmin.cs.lth.se/cs/Personal/Tomas_Akenine-Moller/pubs/tribox.pdf
  // this implementation is not very efficient, but it's more compact and easier
  // to follow, IMO
  //
  // There are lots of speed gains to be had by exploiting the fact that the
  // bounding box faces are all unit vectors
  ...
  Vec3f vEdges[3] = {vVertices[1] - vVertices[0], vVertices[2] - vVertices[1],
  ...
  static const Vec3f vBBFaceNormals[3] = {Vec3f(1, 0, 0), Vec3f(0, 1, 0),
                                          Vec3f(0, 0, 1)};
