...
/**
The main function to take note of is shown below. It performs the entire
optimization as described by the paper.

void FanVertOptimize(   float *pfVertexPositionsIn,   //vertex buffer positions,
3 floats per vertex int *piIndexBufferIn,         //index buffer positions, 3
ints per vertex *in CCW order* int *piIndexBufferOut,        //updated index
buffer (the output of the algorithm) int iNumVertices,             //# of
vertices in the vertex buffer int iNumFaces,                //# of faces in the
index buffer int iCacheSize,               //hardware cache size (usually 12
though 24 are good options)

                              float alpha,                  //constant parameter
to compute lambda term from algorithm float beta,                   //linear
parameter to compute lambda term from algorithm
                                                      //lambda = alpha + beta *
ACMR_OF_TIPSIFY
                                                      //refer to the paper for a
description of these parameters
                                                      //(usually alpha=0.75f and
beta=0.0f are good options)

                        int *piScratch = NULL,        //optional temp buffer for
computations; its size in bytes should be:
                                                      //FanVertScratchSize(iNumVertices,
iNumFaces)
                                                      //if NULL is passed,
function will allocate and free this data

                        int *piClustersOut = NULL,    //optional buffer for the
output cluster position (in faces) of each cluster int *piNumClustersOut = NULL)
//the number of putput clusters

If you don't care about overdraw and just want vertex cache optimization, you
may use:
...
The function below just clusters the mesh. It assumes it is already sorted and
pre-clustered with "hard boundaries" during vertex cache optimization using the
above function. (please see paper for more details)
...
The function below just optimizes for overdraw and returns a "remap" array which
maps the new cluster IDs to the old ones. It is particularly useful for
characters composed of multiple draw calls, as this will give an ordering of
draw calls to attempt to reduce overdraw.
...
float FanVertLinSort(int* piIndexBufferIn, int* piIndexBufferOut, int iNumFaces,
                     int* piScratch, int iCacheSize, int* piClustersOut,
                     int& iNumClusters)
{
  int i = 0;
  ...
  int iCurCachePos =
    1 + iCacheSize;  // so that cache position of 0 is out of cache
  int iCurCachePosFan;
  ...
  // this way, after clustering, we have clusters of full fans around each
  // vertex with each triangle appearing 3 times in the list, once for each
  // adjacent vertex
  // In order to make the code efficient, triangles are uniquely identified by
  // an index
  // into the original buffer. So, id 7 is the second permutation of the third
  // triangle in the input array.
  for (i = 0; i < iNumFaces3; i++)
  {
  ...
  while (lowi < iNumFaces3)
  {
    ...
    // loop through extracting all faces from a vertex fan that were not
    // previously written
    while (i < iNumFaces3 && piIndexBufferIn[piTriList[i]] == id)
    {
      // get triangle id, and starting index of that triangle in original buffer
      // (tri3)
  ...
  int iNumClusters, int* piScratch, int* piRemap = nullptr)
{
  ...
  unsigned int nVertexCrossOrder[2];  // a variable for crossing vertices that
                                      // takes into account winding order
  ...
  for (i = 0; i <= iNumFaces; i++)
  {
    ...
    Vector vNormal = cross(
      pvVertexPositionsIn[p[nVertexCrossOrder[0]]] - pvVertexPositionsIn[p[0]],
...
void FanVertOptimize(
  float* pfVertexPositionsIn,  // vertex buffer positions, 3 floats per vertex
  ...
  int* piNumClustersOut = nullptr)  // the number of putput clusters
{
  ...
  int iNumClusters;
  float lambda =
    FanVertLinSort(piIndexBufferIn, piIndexBufferTmp, iNumFaces, piScratch,
                   iCacheSize, piClustersIn, iNumClusters);

  lambda = alpha + beta * lambda;

  int iNumClustersOut = OverdrawOrderPartition(
    piIndexBufferTmp, iNumFaces, piClustersIn, iNumClusters, iCacheSize, lambda,
    piClustersTmp, piScratch);

  OverdrawOrder(piIndexBufferTmp, piIndexBufferOut, iNumFaces,
...
float FanVertOptimizeVCacheOnly(int* piIndexBufferIn, int* piIndexBufferOut,
                                int iNumVertices, int iNumFaces, int iCacheSize,
                                int* piScratch, int* piClustersOut,
                                int* iNumClusters)
{
  bool bMalloc = false;
...
void FanVertOptimizeClusterOnly(int* piIndexBufferIn, int iNumVertices,
                                int iNumFaces, int iCacheSize, float lambda,
                                int* piClustersIn, int iNumClusters,
                                int* piClustersOut, int* iNumClustersOut,
                                int* piScratch)
{
  bool bMalloc = false;
  ...
  *iNumClustersOut = OverdrawOrderPartition(
    piIndexBufferIn, iNumFaces, piClustersIn, iNumClusters, iCacheSize, lambda,
    piClustersOut, piScratch);
...
void FanVertOptimizeOverdrawOnly(float* pfVertexPositionsIn,
                                 int* piIndexBufferIn, int* piIndexBufferOut,
                                 int iNumVertices, int iNumFaces,
                                 TootleFaceWinding eFrontWinding,
                                 int* piClustersIn, int iNumClusters,
                                 int* piScratch, int* piRemap)
{
  bool bMalloc = false;
  ...
  OverdrawOrder(piIndexBufferIn, piIndexBufferOut, iNumFaces,
                pfVertexPositionsIn, iNumVertices, eFrontWinding, piClustersIn,
                iNumClusters, piScratch, piRemap);
