...
static void AddNeibToQueue(priority_queue<QNode, vector<QNode>, greater<>>& q,
                           Mesh& /*mesh*/, std::vector<int>& fixed,
                           std::vector<float>& cost, std::vector<int>& cluster,
                           std::vector<Vector3>& clusterNormal, int f, int ff,
...
static int MoveFaces(Mesh& mesh, std::vector<int>& seeds,
                     ...
                     std::vector<Vector3>& tn, float& fAvgDist)
{
  ...
  if (f == -1)
  {
    ...
    // if this happens, then we should find the first face not assigned to a
    // cluster, and return that one this will cause us to add a new seed to that
    // face, effectively jumping across the connectivity gap

    // if the entire input mesh happens to be disconnected, then this should
    // converge to one face per cluster
    std::uint32_t i;
  ...
  for (int i = 0; i < static_cast<int>(mesh.t().size()); i++)
  {
    if (!static_cast<bool>(fixed[i]) && vis[i] < lowvisi)
    {
      lowvisi = vis[i];
      lowvisf = i;
    }
                      ...
                      std::vector<Vector3>& tc)
{
  ...
  for (int i = 0; i < static_cast<int>(mesh.t().size()); i++)
  {
    ...
    for (int j = 0; j < static_cast<int>(mesh.ae(i).size()); j++)
    {
      ...
      if (ff != -1 &&  // not mesh boundary
          (cluster[ff] != cluster[i] || !static_cast<bool>(fixed[i]) ||
           !static_cast<bool>(fixed[ff])))
      {
        cost[i] = 0;
        q.push(QNode(.0f, i));
        break;
  ...
  for (int i = 0; i < static_cast<int>(mesh.t().size()); i++)
  {
    ...
    {
      // do not change seeds if the seed we have chosen is the seed of another
      // cluster
      bool bMove = true;
...
/// \param nClusters The number of clusters to create.  If 0, then automatic
/// clustering is used, and nClusters is set
///                  to the number of clusters that are generated
/// \param cluster  An array that will receive the cluster ID to assign to each
/// face \return  One of the ClusterResult return codes
ClusterResult Cluster(Soup* soup, std::uint32_t& nClusters,
                      std::vector<int>& cluster)
{

  // compute face normals
  if (soup->ComputeNormals() == 0)
  {
    return CLUSTER_OUT_OF_MEMORY;
  }

  const auto nFaces = static_cast<int>(soup->t().size());
  ...
  if (mesh.ComputeVT(meshVT) == 0)
  {
    return CLUSTER_OUT_OF_MEMORY;
  }

  // compute per-face adjacency
  // allocate the AE array ahead of time so that we can detect out-of-memory
  // conditions
  mesh.ae().resize(mesh.t().size());

  // computeAE should now only fail if the mesh is non-manifold
  if (mesh.ComputeAE(meshVT) == 0)
  {
    return CLUSTER_OUT_OF_MEMORY;
  }
  ...
  if (mesh.ComputeTriNormals(tn) == 0)
  {
    return CLUSTER_OUT_OF_MEMORY;
  }
  ...
  if (mesh.ComputeTriCenters(tc) == 0)
  {
    return CLUSTER_OUT_OF_MEMORY;
  }
  ...
  {
    nClusters = (std::uint32_t)mesh.t().size();
  }
  ...
  for (int i = 0;; i++)
  {
    ...
    // if the mesh has lots of connected components then we can get a case like
    // this
    bHasUnassignedFaces = false;
    auto nSeeds = (std::uint32_t)seeds.size();
...
/// \param pRemapArray  An array that will receive the face re-mapping.  May NOT
/// be NULL as it is used internally for temporary storage \return True if
