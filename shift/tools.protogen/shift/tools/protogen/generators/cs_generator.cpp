...
//{
//  return std::make_tuple(std::cref(scope), std::cref(reference), mode,
//  ptrType);
//}
...
//    if (scope.has_attribute("cs_name") &&
...
//    interface->attribute(
//      "csDeserializer",
//      "Deserializer_" + relative_name(*interface, _source->global_scope(),
//      "_"), attribute_assignment::overwrite);
//    auto interfaceName =
//    interface->attribute<std::string>("cs_name").substr(1); for (auto* message
//    : interface->messages)
//    {
...
//               << make_tuple(scope, enumeration->base, print_mode::type) <<
//               br;
...
//                     << alias->name << "("
//                     << make_tuple(scope, *argumentReference,
//                     print_mode::type)
//                     << " value)" br;
//            *_source << indent << "{" br;
...
//                   << "), ProtoType(typeof("
//                   << make_tuple(scope, field.reference,
//                   print_mode::proto_type)
//                   << "))] public ";
//        }
//        if (field.has_attribute("csEntityCollection"))
//          *_source << "virtual ";
...
//  if (message.base)
//  {
//    firstField = writeMessageAsParameterList(scope, *message.base,
//    firstField);
//  }
//  for (auto& field : message.fields)
//  {
...
//  for (auto* interface : scope.interfaces)
//  {
//    auto deserializerName =
//    interface->attribute<std::string>("csDeserializer");
...
//    auto* service_interface = service->is_provider ?
...
//    *_source << indent << "    base("
//             << (service->is_provider ? "true" : "false") << ", 0x" <<
//             std::hex
//             << std::setfill('0') << std::setw(8)
//             << service->attribute<std::uint32_t>("uid") << ", service_tag)"
//             br;
//    *_source << indent << "{" br << inc_indent;
...
//      *_source << indent << "stream.Read(out " << relative_name(*message,
//      scope)
//               << " message, null, new Group());" br;
//      *_source << indent
//               << "if (ServiceHost.Instance.DebugSerializationPerformance)"
//               br;
//      *_source << indent << "{" br << inc_indent;
//      *_source << indent
...
//      *_source << indent
//               << "if (deserializationTime > TimeSpan.FromMilliseconds(50))"
//               br;
//      *_source << indent
...
//             << inc_indent;
//    *_source << indent << "0x" << std::hex << std::setfill('0') <<
//    std::setw(8)
