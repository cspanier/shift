#ifndef SHIFT_MATH_MATRIX3X3_HPP
#define SHIFT_MATH_MATRIX3X3_HPP
...
constexpr auto operator*(const matrix<3, 3, U>& lhs, const vector<3, V>& rhs)
{
  return vector<3, decltype(std::declval<U>() * std::declval<V>())>{
    dot(lhs.row_vector(0), rhs), dot(lhs.row_vector(1), rhs),
    dot(lhs.row_vector(2), rhs)};
}

/// Element-wise matrix-matrix division.
template <typename U, typename V>
constexpr auto operator/(const matrix<3, 3, U>& lhs, const matrix<3, 3, V>& rhs)
{
  // clang-format off
  return matrix<3, 3, decltype(std::declval<U>() / std::declval<V>())>{
    column_major{},
...
constexpr auto operator/(const matrix<3, 3, U>& lhs, V rhs)
{
  // clang-format off
  return matrix<3, 3, decltype(std::declval<U>() / std::declval<V>())>{
    column_major{},
...
template <typename T>
constexpr T determinant(const matrix<3, 3, T>& any_matrix) noexcept
{
...
template <typename T>
constexpr auto inverse(const matrix<3, 3, T>& any_matrix) noexcept
{
...
template <typename T>
constexpr matrix<3, 3, T>::matrix(T fill_value) noexcept
: _columns{{fill_value}, {fill_value}, {fill_value}}
{
}
...
template <typename T>
constexpr T& matrix<3, 3, T>::operator()(std::size_t row, std::size_t column)
{
...
template <typename T>
constexpr T matrix<3, 3, T>::operator()(std::size_t row,
...
template <typename T>
constexpr const typename matrix<3, 3, T>::column_type&
matrix<3, 3, T>::column_vector(std::size_t column) const
{
  return _columns[column];
}

template <typename T>
constexpr typename matrix<3, 3, T>::column_type matrix<3, 3, T>::row_vector(
