...
static file_writer& operator<<(file_writer& stream, const namescope& namescope)
{
...
{
  stream << field.reference << " " << field.attribute<std::string>("cpp_name");
  return stream;
}
...
std::vector<boost::filesystem::path> cpp_generator::output_paths() const
{
  return {cpp_source_output_path, cpp_include_output_path};
}

bool cpp_generator::generate(namescope& root_scope, namescope& /*limit_scope*/)
{
  ...
  for (auto groupBegin = _node_groups.begin(), groupEnd = _node_groups.begin();
    ...
    *_source << "#include \"" << groupName << ".h\"" br;
    ...
    std::function<void(const proto::node*)> visit =
      [&](const proto::node* item) {
        if (visitedGroupDefinitions.find(item) == visitedGroupDefinitions.end())
        {
...
void cpp_generator::preprocess(namescope& scope)
{
  ...
  for (auto* enumeration : scope.enumerations)
  {
    ...
    enumeration->attribute("cpp_def_group_name", decl_group_name(*enumeration));
...
{
  if (reference.as_built_in_type() != nullptr)
  {
    for (const auto& template_argument : reference.arguments)
    {
      const auto* argument_type =
...
void cpp_generator::gather_dependencies(const node& target, const alias& alias,
                                        node_set& required_declarations,
                                        node_set& required_definitions,
                                        bool definition)
{
...
void cpp_generator::write_declaration(const node& node)
{
  ...
  else if (const auto* interface = dynamic_cast<const proto::interface*>(&node))
  {
...
void cpp_generator::write_declaration(const message& message)
{
  ...
  _use_header = true;
  *_header << indent << "struct " << message.attribute<std::string>("cpp_name")
           << ";" << br;
}
...
void cpp_generator::write_definition(const message& message)
{
  ...
  *_header << indent
           << "static constexpr ::shift::serialization::message_uid_t uid = 0x"
  ...
  *_header << indent << "/// Copy constructor." br;
  ...
  {
    *_header << indent << name << "(const " << name << "& other) = delete;" br2;
  }
  ...
  *_source << indent << "shift::core::group_ptr<shift::proto::message> " << name
...
void cpp_generator::write_definition(const service& service)
{
  ...
  _use_header = true;
  _use_source = true;
  *_header << indent << "class " << service_name << " final :" br << inc_indent;
