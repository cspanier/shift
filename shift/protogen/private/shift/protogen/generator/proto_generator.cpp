...
static file_writer& operator<<(file_writer& stream, const type_path& typePath)
{
  bool first = true;
...
static file_writer& operator<<(file_writer& stream, const built_in_type& type)
{
  switch (type)
  {
                            ...
                            const namescope& scope)
{
  ...
  {
    if (interface->attribute<std::string>("source_filename") != source_filename)
      continue;
