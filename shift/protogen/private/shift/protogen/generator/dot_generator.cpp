...
static std::ostream& operator<<(std::ostream& stream, const built_in_type& type)
{
  switch (type)
  {
...
bool dot_generator::generate(namescope& root_scope, namescope& /*limit_scope*/)
{
  _use_services = false;
  ...
  *_source << br;
  ...
  for (const auto& edge : sorted_edges)
  {
    *_source << indent << "\"" << static_cast<const node&>(*edge.first)
             << "\" -> \"" << static_cast<const node&>(*edge.second) << "\"" br;
  }
