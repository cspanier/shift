...
int heap_create(p_heap* H, t_heap_key max)
{
  ...
  *H = h = (p_heap)malloc(sizeof(t_heap));
  ...
  if ((h->node == nullptr) || (h->position == nullptr))
  {
    heap_destroy(H);
    return 0;
  }
...
void heap_dump(p_heap h)
{
  ...
  {
    printf("%d %d (%d), ", (int)h->node[i].key, h->node[i].cost,
           (int)h->position[h->node[i].key]);
  }
  printf("\n");
}
