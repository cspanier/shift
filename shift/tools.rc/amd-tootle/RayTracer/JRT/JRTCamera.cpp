...
JRTPerspectiveCamera::JRTPerspectiveCamera(const Vec3f& position,
                                           const Vec3f& direction,
                                           const Vec3f& up, float fov)
...
void JRTPerspectiveCamera::GetRay(float s, float t, Vec3f* origin,
                                  Vec3f* delta) const
{
...
void JRTPerspectiveCamera::ProjectPoint(const Vec3f& pt, float* s,
                                        float* t) const
{
  // convert point to camera space
  Vec3f pcam = pt - m_position;
  Vec3f camera_space_pt =
    (m_right * pcam.x) + (m_up * pcam.y) + (m_direction * pcam.z);
