...
const std::uint32_t JRTKDTree::MAX_TREE_DEPTH = 28;

JRTKDTree::JRTKDTree()
: m_treeBounds(Vec3f(0, 0, 0), Vec3f(0, 0, 0)), m_pIndexArray(nullptr)
{
}

JRTKDTree::~JRTKDTree()
{
  if (m_pNodeArray != nullptr)
  {
    _aligned_free(m_pNodeArray);
  }

  if (m_pTriArray != nullptr)
  {
...
/// \param pHit     Pointer to a hit info structure that will be set if the ray
/// hits something \param pExcludeMesh
///     Mesh containing the 'exclude' triangle
/// \param nExcludeTri
///     Index of the 'exclude' triangle within pExcludeMesh.  The 'exclude'
///     triangle is ignored during ray traversal to avoid artifacts such as
///     shadow acne, as well as for efficiency
/// \param pNodeStateArray
///     An array of booleans which is used to track CSG node status.  The CSG
///     algorithm requires that we know, at all times during traversal, whether
///     the ray is inside or outside each individual CSG node This array is
///     indexed by CSG node ID.  It should be initialized for the ray origin in
///     advance

bool JRTKDTree::FindFirstHit(const Vec3f& rOrigin, const Vec3f& rDirection,
                             JRTHitInfo* pHit, const JRTMesh* pExcludeMesh,
                             std::uint32_t nExcludeTri,
                             bool* /*pNodeStateArray*/)
{

  pHit->pMesh = nullptr;
  pHit->nIndex = 0;

  // bounding box test to determine ray traversal ranges, and handle trivial
  // rejections
  float tmin, tmax;
  ...
  std::uint32_t nRayID = m_nNextRayID;
  m_nNextRayID++;

  if (m_nNextRayID == 0)
  {
    // handle overflow.  The mailboxes are initialized to 0, so to be fully
    // correct we can't have any mailboxes have ID 0
    m_nNextRayID = 1;
  }
  ...
  Vec3f inv_direction =
    Vec3f(1.0f / rDirection.x, 1.0f / rDirection.y, 1.0f / rDirection.z);

  static float barycentrics[3];

  // rather than using recursion, we're using iteration and handling the stack
  // ourselves
  static float tmin_stack[MAX_TREE_DEPTH];
  static float tmax_stack[MAX_TREE_DEPTH];
  static std::uint32_t node_stack[MAX_TREE_DEPTH];
  static std::uint32_t stack_offs = 0;
  ...
  while (stack_offs > 0)
  {
    ...
    // TODO(OPTIMIZATION):  re-arrange stack to use structures for more
    // localized memory access
    std::uint32_t curr_node = node_stack[stack_offs];
    ...
    while (!node->IsLeaf())
    {
      ...
      // for now, this is just a straight implementation of all the assorted
      // cases
      // TODO:  optimize this properly

      UBYTE axis = node->inner.axis;
      std::uint32_t front = node->inner.front_offset;
      std::uint32_t back = front + 1;
      ...
      //    If the plane cuts through a particular position on the axis (e.g.
      //    x=1) then the D term of the plane equation is the negative of that
      //    position
      //   Because the planes are axis-aligned, it suffices to use one component
      //   of the origin and direction to compute the dot product
      ...
      if (ov > node->inner.position)
      {
        ...
        if (dv >= 0)
        {
          ...
          JRT_ASSERT(thit < 0 || !std::isfinite(thit));
          // traverse front only
          curr_node = front;
        }
        else
    ...
    if (node->leaf.triangle_count > 0)
    {
      ...
      {
        std::uint32_t triIndex = m_pIndexArray[tri];
        ...
        if (RayTriangleIntersect(pTri, rOrigin, rDirection, tmin, t_CurrentHit,
                                 &tval, barycentrics))
        {
          // See if this is the exclude triangle:
          // I've found that it tends to be faster to do this exclusion test
          // after the intersection test rather than before it.  The exclusion
          // test is still necessary to prevent shadow acne etc.
          ...
          // get the normal
          pTri->pMesh->GetInterpolants(pTri->nTriIndex, barycentrics,
                                       &pHit->mNormal, nullptr);
        }
      }
    ...
    // **********************************************************************************
    // DONE VISITING NODE:
    // if we haven't found a hit, we will jump back to the last node where both
    // subtrees get traversed.  We will then trace the other subtree........
    // **********************************************************************************
...
void JRTKDTree::CullBackfaces(const Vec3f& rViewDir, bool bCullCCW)
{
  ...
  {
    std::uint32_t nTriIndex = m_pTriArray[i].nTriIndex;
    m_bBackFacing[i] =
      (DotProduct(viewDir, m_pTriArray[i].pMesh->GetFaceNormal(nTriIndex)) >=
       0);
  }
}

/// This method traces the given ray through the KD tree and locates all ray
/// hits.  The hits are placed into the given array, which may be re-sized if
/// necessary \param rOrigin Ray origin \param rDirection Ray direction \param
/// ppHitArray  An output array which will hold the ray hits.  The array will be
/// re-sized as needed \param pnArraySize  A pointer to the array size \return
/// The number of hits that were found.  Returns JRTKDTree::OUT_OF_MEMORY if out
/// of memory
std::uint32_t JRTKDTree::FindAllHits(const Vec3f& rOrigin,
                                     const Vec3f& rDirection,
                                     ...
                                     std::uint32_t* pnArraySize)
{
  ...
  while (pFrame != traversal_stack /*stack_offs > 0*/)
  {
    ...
    pFrame--;
    std::uint32_t curr_node = pFrame->NextNode;
    tmin = pFrame->tmin;
    tmax = pFrame->tmax;
    ...
    while (!node->IsLeaf())
    {
      ...
      /*
       ...
       if( ov < node->inner.position )
       {
          std::swap<std::uint32_t>( front, back );
       }
    ...
    if (node->leaf.triangle_count > 0)
    {
      ...
      for (int tri = node->leaf.triangle_start; tri < triangle_end; tri++)
      {
        ...
        if (RayTriangleIntersect(pTri, rOrigin, rDirection, tmin, GlobalTMax,
                                 &tval, nullptr))
        {
          ...
          {
            // grow array
            std::uint32_t nOldArraySize = *pnArraySize;
            *pnArraySize = 2 * (*pnArraySize);
            auto* pTemp = new TootleRayHit[*pnArraySize];
...
std::uint32_t JRTKDTree::GetMaxDepth() const
{
  return RecurseMaxDepth(0);
}

std::uint32_t JRTKDTree::GetLeafCount() const
{
  std::uint32_t nLeafs = 0;

  for (std::uint32_t i = 0; i < GetNodeCount(); i++)
  {
...
std::uint32_t JRTKDTree::GetMemoryUsage() const
{
  return sizeof(JRTKDNode) * m_nNodeCount +
         (sizeof(JRTCoreTriangle)) * m_nTriangleCount +
         sizeof(std::uint32_t) * m_nIndexCount;
}

std::uint32_t JRTKDTree::RecurseMaxDepth(std::uint32_t nNode) const
{
  ...
  else
  {
    ...
    std::uint32_t front = pNode->inner.front_offset;
    std::uint32_t back = pNode->inner.front_offset + 1;
    std::uint32_t frontDepth = 1 + RecurseMaxDepth(front);
    std::uint32_t backDepth = 1 + RecurseMaxDepth(back);
