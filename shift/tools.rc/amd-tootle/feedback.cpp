...
PARC arc;  // renamed to arc, because of function redefinition error. Conflicts
           // with name of function "Arc" in windows.h
...
// helper macro to to test for out of memory.  This makes the code below a bit
// more compact
...
int feedback(int nVerts, int nArcs, t_edge* graph, int* order)
{
  ...
  while (1)
  {
    ...
    Output((int)heap_gettop(Heap, nullptr));
