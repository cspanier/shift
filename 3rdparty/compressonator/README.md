...
cmp_status = CMP_ConvertTexture(&srcTexture, &destTexture, &options, &CompressionCallback, nullptr, nullptr);
...
`CMP_EncodeBC7Block(BC7BlockEncoder* encoder  , double in[BC_BLOCK_PIXELS][BC_COMPONENT_COUNT], std::uint8_t* out);`
`CMP_EncodeBC6HBlock(BC6HBlockEncoder* encoder, float  in[BC_BLOCK_PIXELS][BC_COMPONENT_COUNT], std::uint8_t* out);`
