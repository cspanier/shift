...
white                std::uint32_t 0ffffffh, 0ffffffh, 0ffffffh, 0ffffffh
                    std::uint32_t 0ffffffh, 0ffffffh, 0ffffffh, 0ffffffh
                    std::uint32_t 0ffffffh, 0ffffffh, 0ffffffh, 0ffffffh
                    std::uint32_t 0ffffffh, 0ffffffh, 0ffffffh, 0ffffffh


whiteblack            std::uint32_t 0ffffffh, 0ffffffh, 0ffffffh, 0ffffffh
                    std::uint32_t 0ffffffh, 0000000h, 0000000h, 0ffffffh
                    std::uint32_t 0ffffffh, 0ffffffh, 0000000h, 0000000h
                    std::uint32_t 0000000h, 0000000h, 0000000h, 0000000h

redblack            std::uint32_t 0ff0000h, 0ff0000h, 0ff0000h, 0ff0000h
                    std::uint32_t 0ff0000h, 0000000h, 0000000h, 0ff0000h
                    std::uint32_t 0ff0000h, 0ff0000h, 0000000h, 0000000h
                    std::uint32_t 0000000h, 0000000h, 0000000h, 0000000h

redsblack            std::uint32_t 0ff0000h, 0aa0000h, 0aa0000h, 0ff0000h
                    std::uint32_t 0ff0000h, 0550000h, 0550000h, 0ff0000h
                    std::uint32_t 0ff0000h, 0ff0000h, 0000000h, 0000000h
                    std::uint32_t 0000000h, 0000000h, 0000000h, 0000000h

redsblack_prog        std::uint32_t 0ff0000h, 0990000h, 0990000h, 0990000h
                    std::uint32_t 0990000h, 0660000h, 0660000h, 0990000h
                    std::uint32_t 0990000h, 0990000h, 0660000h, 0000000h
                    std::uint32_t 0660000h, 0660000h, 0660000h, 0660000h

redsblack2            std::uint32_t 0e00000h, 0e00000h, 0e00000h, 0e00000h
                    std::uint32_t 0c00000h, 0c00000h, 0c00000h, 0c00000h
                    std::uint32_t 0a00000h, 0a00000h, 0a00000h, 0a00000h
                    std::uint32_t 0800000h, 0800000h, 0800000h, 0800000h

greenred            std::uint32_t 0ff0000h, 0ff0000h, 0ff0000h, 0ff0000h
                    std::uint32_t 0ff0000h, 0ff0000h, 0ff0000h, 0ff0000h
                    std::uint32_t 000ff00h, 000ff00h, 000ff00h, 000ff00h
                    std::uint32_t 000ff00h, 000ff00h, 000ff00h, 000ff00h

bluered                std::uint32_t 0ff0000h, 0ff0000h, 0ff0000h, 0ff0000h
                    std::uint32_t 0ff0000h, 0ff0000h, 0ff0000h, 0ff0000h
                    std::uint32_t 00000ffh, 00000ffh, 00000ffh, 00000ffh
                    std::uint32_t 00000ffh, 00000ffh, 00000ffh, 00000ffh

fade                std::uint32_t 0ffffffh, 0ffffffh, 0b0b0b0h, 0707070h
                    std::uint32_t 0ffffffh, 0ffffffh, 0b0b0b0h, 0707070h
                    std::uint32_t 0ffffffh, 0ffffffh, 0d0d0d0h, 0909090h
                    std::uint32_t 0ffffffh, 0ffffffh, 0ffffffh, 0e0e0e0h

col_table            std::uint32_t 000000000h, 0001f001fh, 0f800f800h, 0f81ff81fh
                    std::uint32_t 007e007e0h, 007ff07ffh, 0ffe0ffe0h, 0ffffffffh
                    std::uint32_t 084108410h

ALIGN 16

testval                    equ 0400h
test_val                std::uint16_t testval, testval, testval, testval, testval, testval, testval, testval
...
max_sint32                std::uint32_t 07fffffffh, 07fffffffh, 07fffffffh, 07fffffffh

scale_one_third            std::uint16_t one_third,one_third,one_third,one_third,one_third,one_third,one_third,one_third
stepsize                std::uint16_t step,step,step,step,step,step,step,step
prog_threshold_size        std::uint16_t prog_threshold,prog_threshold,prog_threshold,prog_threshold,prog_threshold,prog_threshold,prog_threshold,prog_threshold

IF WEIGHTING
WEIGHTING_BITS            equ 4                ; If this equals AVG_FRAC_BITS a few extra optimisations kick in
weighting                std::uint16_t 4,16,8,0, 4,16,8,0
unweighting                std::uint16_t 4,1,2,0
round_565_weighted        std::uint16_t 0010h, 0020h, 0020h, 0
ELSE
WEIGHTING_BITS            equ 0
ENDIF

IF UNROUNDING
round_565                std::uint16_t 0040h,0020h,0040h,0    ; This is applied to the average, which is in 8.4 format, so it's 0.5 in 5.7, 6.6, 5.7 format
scale_to_round            std::uint16_t 2048,1024,2048,0        ; This is a 5 6 5 right shift in 8.4 format
round_mask                std::uint16_t 0fff0h, 0fff0h, 0fff0h, 0
ENDIF


scale_8_4_to_565        std::uint16_t 32*16,    64*16, 32*16,    0
clamp_565                std::uint16_t 31,63,31,0
...
avg_round                std::uint16_t avg_round_val,avg_round_val,avg_round_val,avg_round_val,avg_round_val,avg_round_val,avg_round_val,avg_round_val
ENDIF
...
; Temporaries
TMP_TMP                equ    (8*16)            ; Basic tmp area - 8 XMMWORD each contains two std::uint16_t RGB0 source values
...
ELSE    ; X64
    ...
    SAVE_REGS        MACRO
        ...
        ; Set up the tmp pointer aligned on a 16-std::uint8_t boundary
        mov ebp, esp
        and ebp, NOT 0fh
    ENDM
...
AXIS_3COMPONENT        MACRO no_axis
    ...
    ENDIF
        ...
        punpcklwd xmm1,xmm0                            ; The axis is always positive so we can unpack with 0
            ...
            ; See the AXIS_ORDER_ENTRY macro for how we create the 512-std::uint8_t table of negation bits
            and eax,02ah
        ...
        mulps xmm2,xmm2
            ...
            shl eax,3            ; x8, with empty lower bit, is a 16-std::uint8_t aligned pointer to the 32 entry table
...
DXTCV11CompressAlphaBlockSSE2 PROC
        ...
        movdqa xmm3,xmm0
        psrldq xmm0,10                    ; 10-std::uint8_t shift
...
;void __fastcall DXTCV11CompressExplicitAlphaBlockMMX(std::uint8_t block_8[16], std::uint32_t block_dxtc[2]);
    IF X64
...
;void __fastcall DXTCV11CompressBlockSSE2Strided(std::uint32_t *block_32, std::uint32_t *block_dxtc, std::uint32_t input_stride);
...
; void __cdecl DXTCCompressBlockSSE(std::uint32_t *block_32, std::uint32_t *block_dxtc);
