...
void DXTCV11CompressBlock(std::uint32_t block_32[16], std::uint32_t
block_dxtc[2])
{
    int i, k;
    float r, g, b;
    ...
    float unique_recip;                        // Reciprocal of the above for
fast multiplication int index_map[16];                        // The map of
source pixels to unique indices int pixel_count[16];                    // The
number of occurrences of each unique

    float average_r, average_g, average_b;    // The centrepoint of the axis
    float v_r, v_g, v_b;                    // The axis

    float pos_on_axis[16];                    // The distance each unique falls
along the compression axis float dist_from_axis[16];                // The
distance each unique falls from the compression axis float left=0, right=0,
centre=0;                // The extremities and centre (average of left/right)
of uniques along the compression axis float axis_mapping_error=0; // The total
computed error in mapping pixels to the axis

    int retry;                                // Retry count for axis mapping
    int swap;                                // Indicator if the RGB values need
swapping to generate an opaque result int blocktype=0;
...
-------------------------------------------------------------------------------------
    {
        // Find the array of unique pixel values and sum them to find their
average position float *up = (float *)uniques; int current_pixel, firstdiff;
    ...
    //
-------------------------------------------------------------------------------------
    // For each component, reflect points about the average so all lie on the
same side
    // of the average, and compute the new average - this gives a second point
that defines the axis
    // To compute the sign of the axis sum the positive differences of G for
each of R and B (the
    // G axis is always positive in this implementation
    //
-------------------------------------------------------------------------------------
    // An interesting situation occurs if the G axis contains no information, in
which case the RB
    // axis is also compared. I am not entirely sure if this is the correct
implementation - should
...
#if 1    // Experimental code for infinite partition compression
    {
        ...
        // We perform the partition based on a plane in the 3D colourspace. The
plane is defined by
        // a point and a normal.

        // Finding this plane is, unsurprisingly for me, based off a hand-waving
argument that seems to
        // solve all the simple cases and is usually bafflingly found to do OK
on the difficult ones.
        ...
        // This gives the direction of the normal vector but not the signs of
each component. There
        // are 8 possibilities; for each we calculate the sum of the distances
from the plane of all
        // considered points. There are two cases of interest, that for which
the sum of distances is
        // maximum, and that for which the sum of distances is minimum.

        // These two possibilities define two different situations. If there are
two groups of points,
        // widely spaced, that each compress reasonably we should pick the axis
with the maximum
        // sum of distances. If all the points together are close to a single
line, splitting by
        // this method will just generate two nearly identical lines. Instead,
we should pick the axis
        // with the minimum sum of distances, which will create two close but
not identical lines,
        // probably parallel in the colourspace.

        // Which of these cases is in use can be determined by the ratio of
minimum to maximum. If the
        // minimum and maximum are 'close' then we are in a case where the
maximum plane is the best used. If
        // the minimum is only a small fraction of the maximum then the minimum
plane is best used. Intermediate
        // cases imply that neither is a particularly good representation of the
block (this indicates
        // a possible compression metric for the new system).

        // It is not known what ratio will work for this, it will probably be
determined by trial-and-error.
        // It is possible the ratio of choice will vary depending on the number
of pixels in the
        // groups (for example, one highly different pixel compared to a
reasonably uniform line should
        // likely be represented by maximum plane).

        // The minimum plane can be ignored initially. It is never erroneous to
only use the
        // maximum plane, but using the minimum plane gives the ability to
improve quality on those
        // blocks on which DXTC is already very good. (It seems likely that
Andy's exhaustive search
        // algorithm is finding these cases already, and as such attempts to
compare this algorithm with
        // exhaustive search will likely not do too well until the minimum case
is also handled.)
        ...
        std::uint32_t maxvector=0, minvector=0, vector=0;

        assert(unique_pixels == 16);    // vector is confusing to interpret if
this isn't true...
        ...
        // A refinement might be to check if there are any points which have
ended up just on one side
        // of the partition when they would be better on the other. I think this
is unlikely to be a
        // major issue
    ...
    for(retry = 0; retry<2; retry++)
    {
        ...
        // From Foley & Van Dam: Closest point of approach of a line (P + v) to
a point (R) is
        //                            P + ((R-P).v) / (v.v))v
        // The distance along v is therefore (R-P).v / (v.v)
        // (v.v) is 1 if v is a unit vector.
        //
        // Calculate the extremities at the same time - these need to be
reasonably accurately
        // represented in all cases
        //
        // In this first calculation, also find the error of mapping the points
to the axis - this
        // is our major indicator of whether or not the block has compressed
well - if the points
        // map well onto the axis then most of the noise introduced is
high-frequency noise
        ...
        for(i=0; i<unique_pixels; i++)
        {
            // Compute the distance along the axis of the point of closest
approach pos_on_axis[i] = ((uniques[i][0]-average_r) * v_r) +
...
// Latest enhancements / bug fixes seem to have rendered this redundant
        ...
        if (retry == 0)
        {
            // Check if our axis is actually a reasonable one, by clustering the
two end
            // point groups (of the four that would be present in the final
map). If there
            // is a wildly differing axis distance average between the two
clusters then we
            ...
            if (new_left != new_right)
            {
                ...
                error = (float) (fabs(cluster_error[new_left]) -
fabs(cluster_error[new_right])); error *= error; if (error > 50000.0f)        //
Trial-and-error value...
    ...
    //
-------------------------------------------------------------------------------------
    // Now we have a good axis and the basic information about how the points
are mapped
    // to it
...
//#define EXACT_ENDPOINT_DETERMINATOR
#ifdef EXACT_ENDPOINT_DETERMINATOR
    {
        ...
        // G == the extent of the position of the endpoint either side of the
centre
        ...
        // For any value of i:
        // E = SUM(SQUARED(Pn - G)) + SUM(SQUARED(Pm - G/3)) where n goes from 0
to (i-1) and m goes from (i) to 15
        // E = SUM(SQUARED(Pn)) - 2*SUM(t1*Pn*G) + 16*t2*SQUARED(G) where n goes
from 0 to 15
        // (t1 and t2 are the form factors to take accound of the relative
numbers of the G and G/3 comparators)
        // E is a minimum when dE/dG == 0
        // dE/dG = 2*t2*G - 2*t1*16*SUM(Pn)
        // G == SUM(Pn) * (t1/t2) / 16 == a constant * the average value of Pn

        // 2G/3 then needs to be clamped between P(i) and P(i+1). This may be
impossible if P(i) and P(i+1) have
        ...
        // Performance:
        // The evaluation of the minimum error point is easy as it is simply
(constant * average). Calculating
        // the actual error at that point is a little more expensive but not
prohibitively so. However, to check
        // if the configuration is valid will require a sorted array of points,
the generation of which will
        // certainly be relatively expensive, but can be fairly easily and very
quickly be implemented with a
        // binary sort using a temporary array.
        // We can probably also reuse this information when building the
clusters later.
        ...
        for(i=0; i<16; i++)
        {
            ref[i] = i;        // Keep the references; we can then use this to
help with the clustering p[i] = (float)fabs(pos_on_axis[i]); average_p += p[i];
...
#define epsilon (0.01f)
        for(i=0; i<16; i++)
        {
            // If this cannot be clamped to a valid configuration, dump it
straight away.
            // (Actually not sure if this is necessary. The maxima/minima rules
may automatically
...
#define PROGRESSIVE_REFINEMENT
#ifdef PROGRESSIVE_REFINEMENT
    {
        ...
        // output image by trying to find a better overall match for the
endpoints
        // than the first-guess solution found so far (which is just to take the
ends.

        // The method is to move the endpoints inwards until a local minima is
found.
        // This provides quite a significant improvement in image quality.

        // In fact there are six degrees of freedom that could be tested here -
inc/dec
        // left, inc/dec right, move both in, move both out). Move both out will
never
        // need to be tested as we already start at this worst case (both as far
out as
        // possible). Similarly, if every 'either in, both in' case is tested
individually
        // then that also eliminates all 'move out' cases where a single step is
all that
        // is needed

        // Checking the additional modes beyond the first does not provide much
enhancement

        // Changing the stepsize to be additive instead of multiplicative makes
visual image
        // quality very marginally worse, for about the same performance.

        float error4, maxerror, v[5];
        float stepsize = 0.95f;            // Compromise value, seems to produce
good results in most cases while only being a small speed hit std::uint32_t
bit4; float division4; float oldleft, oldright; int mode, bestmode; int first;
#ifdef TRY_3_COLOR
        ...
        do
        {
            for(bestmode=-1,mode=0; mode<1; mode++)        // In my experience
modes other than mode 0 hardly ever get a hit, and it costs nearly 30%
performance to test the other two
            {
                if (!first)
                {
                    switch(mode)
...
#if TRY_3_COLOR
                    // Endpoints (indicated by block > average) are 0 and 1,
while
                    // interpolants are 2 and 3
                    if (fabs(b) >= division4)
                        bit4 = 0;
...
#elif 0
                    {
                        ...
                        bit4 = (*(std::uint32_t *)&d>>30) & 2;
                        d = (b - centre);
                        bit4 |= (*(std::uint32_t *)&d>>31)^1;
                    }
...
#endif
                if (bestmode != -1);
                    maxerror -= 5.0f;    // Errors smaller than a certain
epsilon should be ignored - this prevents unnecessary and infinite loops
...
twitchy. A
    // straight rounded average is not correct, as the decompressor 'unrounds'
by replicating
...
pretty good in terms of
    // error measure, but creates a visual colour and/or brightness shift
relative to the original image)
    // The method used here is to apply a centre-biased rounding dependent on
the input value, which was
    // (mostly by experiment) found to give minimum MSE while preserving the
...
-------------------------------------------------------------------------------------
    {
        ...
        // Force to be a 4-colour opaque block - in which case, c0 is greater
than c1 if (blocktype == 4)
        {
            if (c0 < c1)
            {
                t = c0;
            ...
            else if (c0 == c1)
            {
                ...
                // avoiding accidentally setting some transparent pixels into
the block for(i=0; i<unique_pixels; i++) pos_on_axis[i] = left; swap = 0;
            }
            else
                swap = 0;
        }
...
-------------------------------------------------------------------------------------
    {
        ...
        if (blocktype == 4)
        {
            block_dxtc[1] = 0;
            division = right*2.0f/3.0f;
            centre = (left+right)/2;        // Actually, this code only works if
centre is 0 or approximately so
            ...
            if ((block_dxtc[0]&0xffff) != (block_dxtc[0]>>16))
            {
                ...
                for(k1=0; k1<4; k1++)
                {
                    ...
                    for(i1=0; i1<4; i1++)
                    {
                        if ((POS(x            , y+ystep*i1)    < POS(x+  xstep,
y+ystep*i1)) || (POS(x+  xstep    , y+ystep*i1)    < POS(x+2*xstep, y+ystep*i1))
|| (POS(x+2*xstep    , y+ystep*i1)    < POS(x+3*xstep, y+ystep*i1))
                            )
                            break;
                        if ((POS(x+xstep*i1, y)                < POS(x+xstep*i1,
y+  ystep)) || (POS(x+xstep*i1, y+ystep)        < POS(x+xstep*i1, y+2*ystep)) ||
                            (POS(x+xstep*i1, y+2*ystep)        < POS(x+xstep*i1,
y+3*ystep))
                            )
                            break;
                    }
...
// This compressor can only create opaque, 4-colour blocks
void DXTCV11CompressBlockMinimal(std::uint32_t block_32[16], std::uint32_t
block_dxtc[2])
{
    int i, k;
    float r, g, b;
    ...
    float uniques[16][3];                  // The list of unique colours
    int unique_pixels;                     // The number of unique pixels
    float unique_recip;                    // Reciprocal of the above for fast
multiplication int index_map[16];                     // The map of source
pixels to unique indices int pixel_count[16];                   // The number of
...
along the compression axis float dist_from_axis[16];              // The
distance each unique falls from the compression axis float left = 0, right = 0,
centre = 0; // The extremities and centre (average of left/right) of uniques
along the compression axis float axis_mapping_error = 0;          // The total
computed error in mapping pixels to the axis

    int swap;                              // Indicator if the RGB values need
swapping to generate an opaque result
...
average position float *up = (float *)uniques; int current_pixel, firstdiff;
    ...
    //
-------------------------------------------------------------------------------------
    // (4) For each component, reflect points about the average so all lie on
the same side
    // of the average, and compute the new average - this gives a second point
that defines the axis
    // To compute the sign of the axis sum the positive differences of G for
...
point (R) is
    ...
    // (v.v) is 1 if v is a unit vector.
    //
    // Calculate the extremities at the same time - these need to be reasonably
accurately
    // represented in all cases
    //
    // In this first calculation, also find the error of mapping the points to
the axis - this
    // is our major indicator of whether or not the block has compressed well -
if the points
    // map well onto the axis then most of the noise introduced is
high-frequency noise
    {
...
approach pos_on_axis[i] = ((uniques[i][0] - average_r) * v_r) +
    ...
    //
-------------------------------------------------------------------------------------
    // (7) Now we have a good axis and the basic information about how the
points are mapped
    // to it
    // Our initial guess is to represent the endpoints accurately, by moving the
average
...
-------------------------------------------------------------------------------------
    {
        ...
        // (blocktype == 4)
        {
            block_dxtc[1] = 0;
            division = right*2.0f / 3.0f;
            centre = (left + right) / 2;        // Actually, this code only
works if centre is 0 or approximately so
            ...
            if ((block_dxtc[0] & 0xffff) != (block_dxtc[0] >> 16))
            {
                ...
                for (k1 = 0; k1<4; k1++)
                {
                    ...
                    for (i1 = 0; i1<4; i1++)
                    {
                        if ((POS(x, y + ystep*i1)                < POS(x +
xstep, y + ystep*i1)) || (POS(x + xstep, y + ystep*i1)        < POS(x + 2 *
xstep, y + ystep*i1)) || (POS(x + 2 * xstep, y + ystep*i1)    < POS(x + 3 *
xstep, y + ystep*i1))
                            )
                            break;
                        if ((POS(x + xstep*i1, y)                < POS(x +
xstep*i1, y + ystep)) || (POS(x + xstep*i1, y + ystep)        < POS(x +
xstep*i1, y + 2 * ystep)) || (POS(x + xstep*i1, y + 2 * ystep)    < POS(x +
xstep*i1, y + 3 * ystep))
                            )
                            break;
                    }
