...
inline void GetProjectedCoords(const double* P, unsigned int max_normal_comp,
                               double& Pu, double& Pv)
{
...
inline void GetProjectedCoords(const float* P, unsigned int max_normal_comp,
                               double& Pu, double& Pv)
{
  Pu = UCOMP(P, max_normal_comp);
  Pv = VCOMP(P, max_normal_comp);
...
inline void GetProjectedCoords(const SSEVec4& phit,
                               unsigned int max_normal_comp, SSEVec4& out)
{
...
bool SegTest(float segU1, float segU2, float segV1, float segV2, float Pu,
             float Pv)
{
  // U = aU1 + (1-a)U2
  // V = aV1 + (1-a)V2
  ...
  assert(!std::isnan(alpha) && std::isfinite(alpha));
...
const float* direction, float tmin, float tmax, float* tout, float*
barycentrics_out )
{
    std::uint32_t nIndexW = WCOMPINDEX( pTri->max_normal_comp );
    std::uint32_t nIndexU = UCOMPINDEX( pTri->max_normal_comp );
    std::uint32_t nIndexV = VCOMPINDEX( pTri->max_normal_comp );
    ...
    _asm
    {
        ...
        // if xmm0 is 0, then we have an inf, and we should get out now (this is
a rare case but it happens) xorps   xmm2, xmm2;       // xmm2 = 0 ucomiss xmm0,
xmm2; je RETURN_FALSE; // forward branch, predicted not taken
...
bool RayTriangleIntersect(const JRTCoreTriangle* pTri, const float* origin,
                          ...
                          float* tout, float* /*barycentrics_out*/)
{
  ...
  assert(!std::isnan(t) && std::isfinite(t) && t >= tmin && t <= tmax);
