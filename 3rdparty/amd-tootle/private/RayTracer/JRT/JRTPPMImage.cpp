...
const JRTPPMImage& JRTPPMImage::operator=(const JRTPPMImage& img)
{
  ...
  memcpy(this->p_mlpPixels, img.p_mlpPixels,
         sizeof(PIXEL) * GetWidth() * GetHeight());
...
void ISkipToToken(FILE* fp)
{
  ...
  while ((feof(fp) == 0) && !hit_token)
  {
    ...
    else
    {
      ...
      // we need that character we just read, so backtrack so we're pointed at
      // it
      ungetc(c, fp);
    }
  }
...
bool JRTPPMImage::LoadFile(const char* filename)
{
  ...
  /*
      For reference, here is the PPM image format description, direct from the
     ppm man page.

      A "magic number" for identifying the file type. A ppm file's magic number
     is the two characters "P3". Whitespace (blanks, TABs, CRs, LFs). A width,
     formatted as ASCII characters in decimal. Whitespace. A height, again in
     ASCII decimal. Whitespace. The maximum color-component value, again in
     ASCII decimal. Whitespace. Width * height pixels, each three ASCII decimal
     values between 0 and the specified maximum value, starting at the top-left
      corner of the pixmap, proceeding in normal English reading order. The
     three values for each pixel represent red, green, and blue, respectively; a
     value of 0 means that color is off, and the maximum value means that color
     is maxxed out. Characters from a "#" to the next end-of-line are ignored
  ...
  */
  ...
  bool at_bits = false, got_width = false, got_height = false,
       got_maxval = false;

  while (!at_bits && (feof(fp) == 0))
  {
    ISkipToToken(fp);
  ...
  if ((feof(fp) != 0) && (!got_width || !got_height || !got_maxval))
  {
    fclose(fp);
    return false;
  }
  ...
  auto* bytes = (unsigned char*)malloc(3 * width * height);
