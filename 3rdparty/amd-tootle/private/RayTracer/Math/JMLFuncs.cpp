                        ...
                        SSEVec4& rVecOut)
{
  ...
  auto W = VEC4_W(res);
  SSEVec4 zero = SSEVecZero();
  auto one = SSEVec4(1.0f);

  if ((SSEVecIsXNEqual(W, zero) != 0) && (SSEVecIsXNEqual(W, one) != 0))
  {
    res *= SSEVecRCP(W);
  }
...
void Vec4TransformVector(const SSEVec4& rVector, const Matrix4f* pMatrix,
                         SSEVec4& rVecOut)
{
                        ...
                        const float barycentrics[3])
{
  ...
  return (v1 * barycentrics[0]) + (v2 * barycentrics[1]) +
         (v3 * barycentrics[2]);
}
