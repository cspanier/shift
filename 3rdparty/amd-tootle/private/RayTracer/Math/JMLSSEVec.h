...
public:
  ...
  // inline SSEVec4 operator*( float rhs ) const { return SSEVec4(
  // _mm_mul_ps(vec128, _mm_load1_ps(&rhs)) ); }; inline SSEVec4& operator*=(
  // float rhs )  { vec128 = _mm_mul_ps(vec128, _mm_load1_ps(&rhs)); return
  // *this; };
...
// swizzling macros
#define VEC4_SWIZZLE(vec, x, y, z, w) \
  SSEVec4(_mm_shuffle_ps(vec.vec128, vec.vec128, _MM_SHUFFLE(x, y, z, w)))
...
inline SSEVec4 SSEVecRCP(const SSEVec4& a)
{
  ...
  return SSEVec4(_mm_sub_ps(_mm_add_ps(Ra0, Ra0),
                            _mm_mul_ps(_mm_mul_ps(Ra0, a.vec128), Ra0)));
};
...
inline int SSEVecIsXSet(const SSEVec4& vec)
{
  ...
  // and comparisons also return nan on true.  To fix it, we're turning nan
  // into 1.0f which will work when comparing against zero
...
{
  auto x(VEC4_X(states));
  auto y(VEC4_Y(states));
  auto z(VEC4_Z(states));
  auto w(VEC4_W(states));
...
inline SSEVec4 SSEVecSelect(const SSEVec4& condition, const SSEVec4& val,
                            const SSEVec4& target)
{
